...
function ListReceipts() {
    ...
    const sortedAndFilteredReceipts = useMemo(() => {
      const filteredReceipts = receipts.filter(receipt => {
          // Filtering logic...
          return !searchTerm || receipt.someProperty.toLowerCase().includes(searchTerm.toLowerCase());
      });
...
const requestSort = (key) => {
  setSortConfig(currentConfig => {
      // Toggle direction if the same key is clicked, otherwise sort in ascending
      const isAsc = currentConfig.key === key && currentConfig.direction === 'ascending';
